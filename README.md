...
| **`search_memory`** | Search memory by combined summary and content similarity | `document_id` (integer, optional) `query_summary` (string) `query_content` (string) `min_distance` (number, optional, default value: 0.008) `limit` (integer, optional) | Vectorizes the query texts separately, performs hybrid retrieval on `summary` and `content`, and filters out low-relevance results by distance threshold (default enabled). |