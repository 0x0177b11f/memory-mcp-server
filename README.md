...
| **`search_memory`** | Search memory by summary and content similarity | `document_id` (integer, optional) `query_summary` (string) `query_content` (string) `min_distance` (number, optional, default value: 0.008) `limit` (integer, optional) | Vectorizes both summary and content queries, performs hybrid retrieval on both fields in `memory_items` (optionally limited to a document collection), then fuses and filters results by RRF relevance score threshold (default enabled). |
| **`search_memory_summary`** | Search memory by summary similarity | `document_id` (integer, optional) `query_text` (string) `min_distance` (number, optional, default value: 0.008) `limit` (integer, optional) | Vectorizes the query text, performs hybrid retrieval based on `summary` in `memory_items` (optionally limited to a document collection), then filters out low-relevance results by RRF relevance score threshold (default enabled). |
| **`search_memory_content`** | Search memory by content similarity | `document_id` (integer, optional) `query_text` (string) `min_distance` (number, optional, default value: 0.008) `limit` (integer, optional) | Vectorizes the query text, performs hybrid retrieval based on `content` in `memory_items` (optionally limited to a document collection), then filters out low-relevance results by RRF relevance score threshold (default enabled). |